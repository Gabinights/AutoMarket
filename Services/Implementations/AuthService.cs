        ...
        IMemoryCache cache)
	{
		...
		public async Task<LoginResultDto> LoginAsync(LoginDto dto)
		{
			...
			if (await _userManager.IsInRoleAsync(user, Roles.Vendedor))
			{
				...
				{
					_logger.LogWarning("Vendedor {Email} tentou entrar mas est� com status {Status}", user.Email, vendedor.Status);
					await _signInManager.SignOutAsync();
		...
		public async Task<ConfirmEmailResultDto> ConfirmEmailAsync(string userId, string token)
		{
			...
			{
				errors.Add("Link de confirma��o inv�lido ou incompleto.");
			...
			if (user == null)
			{
				errors.Add("Utilizador n�o encontrado no sistema.");
				return new ConfirmEmailResultDto(false, errors);
			}
			...
			if (!result.Succeeded)
			{
				...
				_logger.LogWarning("Falha na confirma��o de email para {UserId}: {Errors}", userId, string.Join(", ", errors));
				return new ConfirmEmailResultDto(false, errors);
			}
